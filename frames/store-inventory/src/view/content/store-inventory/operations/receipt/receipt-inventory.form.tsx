...
import { schema, type ReceiptInventoryFormData } from './form.schema.ts';

import s from './default.module.scss';

export const ReceiptInventoryForm: React.FC = () => {
  const data = useLoaderData(StoreInventoryContextControllerInterface);
  const submit = useSubmit(ReceiptInventoryControllerInterface);
  const methods = useForm<ReceiptInventoryFormData>({
    mode: 'onChange',
    defaultValues: {
      quantity: 1,
      reason: '',
    },
    resolver: yupResolver(schema) as Resolver<ReceiptInventoryFormData>,
  });
  ...
  return (
    <FormProvider {...methods}>
      <form id={STORE_INVENTORY_FORM_ID} className={s.wrapper} onSubmit={handleSubmit}>
        <InventoryOperationFields inProcess={submit.inProcess} quantityLabel={'Количество поступления'} />
      </form>
    </FormProvider>
  );
};

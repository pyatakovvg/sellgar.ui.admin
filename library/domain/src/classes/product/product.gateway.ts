...
export class ProductGateway {
  ...
  async getAll(): Promise<ResultEntity> {
    const result = await this.httpClient.get<ResultEntity>(import.meta.env.VITE_GATEWAY_API + '/v1/products');
  ...
  async getByUuid(uuid: string): Promise<ProductEntity> {
    const result = await this.httpClient.get<ProductEntity>(import.meta.env.VITE_GATEWAY_API + '/v1/products/' + uuid);
  ...
  update(uuid: string, body: any) {
    return this.httpClient.put(import.meta.env.VITE_GATEWAY_API + '/v1/products/' + uuid, body);
  }

  create(body: any) {
    return this.httpClient.post(import.meta.env.VITE_GATEWAY_API + '/v1/products', body);
  }
}

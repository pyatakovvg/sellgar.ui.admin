import { initializeObservableEntity } from '../../runtime/entity-observer';

const ENTITY_METADATA_KEY = Symbol('tiyn-app:reactive:entity:metadata');
...
export const isEntityConstructor = (value: unknown): value is EntityConstructor => {
  return typeof value === 'function' && Reflect.hasMetadata(ENTITY_METADATA_KEY, value);
};
...
export const getEntityIdentity = <TEntity extends object>(entity: TEntity): EntityIdentity => {
  const constructor = entity.constructor as EntityConstructor<TEntity>;
...
export function Entity<TIdentityProperty extends PropertyKey>(
  ...
  const metadata: EntityMetadata<TIdentityProperty | 'id'> = Object.freeze({

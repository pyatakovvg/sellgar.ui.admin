...
export const RouterHost: React.FC<IProps> = (props) => {
  ...
  return (
    ...
    >
      {renderLayouts(definition.layouts, renderRouterContent(props, components, props.tree?.snapshot ?? snapshot))}
    </RuntimeErrorBoundary>
  );
};
...
const renderRouterContent = (
  ...
  return renderRoutePath(
    components,
    branch.routes,
    branch.pendingLocalChange?.commonRouteCount ?? null,
    props.presentation,
    0,
  );
};
...
const renderRoutePath = (
  ...
  return (
    <RouteHost
      ...
      runtime={runtime}
    >
      {renderRoutePath(components, routes, pendingAfterRouteCount, presentation, index + 1)}
    </RouteHost>
  );
};

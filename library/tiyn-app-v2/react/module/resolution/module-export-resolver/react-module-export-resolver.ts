...
export class ReactModuleExportResolver implements ModuleExportResolverInterface<ModuleMetadata> {
  resolve(moduleExports: ModuleExports) {
    const module = resolveReactModuleExport(moduleExports);
...
const resolveReactModuleExport = (moduleExports: ModuleExports): ModuleConstructor => {
  ...
  if (candidates.length === 0) {
    throw new Error(`Экспорт React Module не найден. Экспорты: ${exportedNames}.`);
  }

  throw new Error('Пакет должен экспортировать ровно один класс React @Module.');
};
